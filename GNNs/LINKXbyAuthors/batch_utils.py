...
#         print(self.edge_index.shape, self.edge_index)
#         num_edges = self.edge_index.shape[1] #sid
...
def make_loader(args, dataset, idx, mini_batch=True, device=torch.device('cpu'), test=False, num_workers=-1):
    # num_workers=-1 -> use a modest default to overlap host-side batch prep with GPU work
    ...
    if not mini_batch:
        ...
        loader = RandomNodeSampler(tg_data, num_parts=1, shuffle=True, num_workers=num_workers)
        return loader
        
    if args.train_batch == 'cluster':
        tg_data = nc_dataset_to_torch_geo(dataset, idx, device=device)
        cluster_data = ClusterData(tg_data, num_parts=args.num_parts)
        loader = ClusterLoader(cluster_data, batch_size=args.cluster_batch_size, shuffle=True, num_workers=num_workers)
        
    elif args.train_batch == 'graphsaint-node':
        tg_data = nc_dataset_to_torch_geo(dataset, idx, device=device)
        if not test:
            loader = GraphSAINTNodeSampler(tg_data, batch_size=args.batch_size, shuffle=True, num_workers=num_workers, num_steps=args.saint_num_steps)
        else:
            loader = RandomNodeSampler(tg_data, num_parts=args.test_num_parts, shuffle=True, num_workers=num_workers)
            
    elif args.train_batch == 'graphsaint-edge':
        tg_data = nc_dataset_to_torch_geo(dataset, idx, device=device)
        if not test:
            loader = GraphSAINTEdgeSampler(tg_data, batch_size=args.batch_size, shuffle=True, num_workers=num_workers, num_steps=args.saint_num_steps)
        else:
            loader = RandomNodeSampler(tg_data, num_parts=args.test_num_parts, shuffle=True, num_workers=num_workers)
        ...
        tg_data = nc_dataset_to_torch_geo(dataset, idx, device=device)
        loader = RandomNodeSampler(tg_data, num_parts=args.num_parts, shuffle=True, num_workers=num_workers)
