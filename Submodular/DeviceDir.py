def get_directory(log = False):
    ...
    try:
        uname_result = os.uname()[1]
        if uname_result.find('gilbreth')==0: ##if not darwin(mac/locallaptop)
            DIR='/scratch/gilbreth/das90/Dataset/'
        elif uname_result.find('unimodular')==0:
            DIR='/scratch2/das90/Dataset/'
        elif uname_result.find('Siddharthas')==0:
